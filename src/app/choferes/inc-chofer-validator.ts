    ...
    static createValidator( ) {
        ...
        return (AC: AbstractControl) => {
            ...
            if(  AC.get('fInicio').value && AC.get('hInicio').value
                ...
                let fInicio = moment( AC.get('fInicio').value + ' ' + AC.get('hInicio').value, 'YYYY-MM-DD HH:mm').toDate();
                let fFin = moment( AC.get('fFin').value + ' ' + AC.get('hFin').value , 'YYYY-MM-DD HH:mm').toDate();
